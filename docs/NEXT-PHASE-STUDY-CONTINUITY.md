...
7. Use the prescribed Obscura-first UI workflow, falling back to Safari when engine fidelity matters. Final browser acceptance should cover exact links, notes/resume, recovery, reading/compare, real export, print preview, narrow layouts, the effective viewport at 200% desktop zoom, reduced motion, and no-WebGL behavior. Use native Safari for its print surface and primary study flow, and the two-engine deterministic suite for injected storage/rendering failures and the full viewport matrix. Emulation does not establish physical-device acceptance.
