...
Recorded locally on 2026-08-21 against the post-round-1 candidate before main
...
- `VITE_BASE_PATH=/PalDawn/ npm test` passed TypeScript, the production build,
  ...
  and five-step reduced-motion completion.
- Built JavaScript was 334,259 bytes gzip against the existing 500 KiB budget.
- `node pipeline/provenance/run-checks.mjs` passed every valid, invalid, and live
  record. No anatomy/binary asset or dependency was added.
- `npm run licenses` reported 91 packages: 89 allowed, 2 existing MPL review
  entries, and 0 denied. The lockfile and inventory are unchanged from v0.1.0.
...
| Skip navigation | First Tab focused “Skip to introduction”; Enter focused `#intro-title`. After entry the link targets `#flight-controls`. |
...
| Settings and local data | Version-1 settings survived reload; invalid enum/type values fell back to defaults. Reset confirmation cleared both PalDawn keys, removed the stage hash, and returned to an intro with no resume prompt. |
