...
export default function ResearchDesk({ atlas, chosen, onChoose, queue, onQueue }: Props) {
  ...
  const queueEntries = queue.flatMap(item => {
    ...
    return searchable.toLowerCase().includes(normalizedQuery) ? [{ item, topic }] : []
  })
