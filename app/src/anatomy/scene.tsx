...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  renderer.setPixelRatio(Math.min(devicePixelRatio,innerWidth<768?1.5:2));renderer.setClearColor('#f0f1eb');renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=.85;el.appendChild(renderer.domElement);
  ...
  const cameraGoal=new T.Vector3(),targetGoal=new T.Vector3();let framing=false,lastQuality='';
  const frameCamera=(target:T.Vector3,position:T.Vector3)=>{cameraGoal.copy(position);targetGoal.copy(target);if(!ready||latest.current.reducedMotion){camera.position.copy(position);controls.target.copy(target);framing=false;}else framing=true;dirty=true;};
  ...
  const key=new T.DirectionalLight(0xfffaf4,1.4);key.position.set(-2,4,3);key.castShadow=true;key.shadow.mapSize.set(2048,2048);key.shadow.camera.left=-1.5;key.shadow.camera.right=1.5;key.shadow.camera.top=2.5;key.shadow.camera.bottom=-1;key.shadow.camera.near=.1;key.shadow.camera.far=12;key.shadow.bias=-.00015;key.shadow.normalBias=.005;key.shadow.radius=3;scene.add(key);
  const rim=new T.DirectionalLight(0xe9f0ff,.8);rim.position.set(2,2,-3);scene.add(rim);
  const ground=new T.Mesh(new T.CircleGeometry(30,96),new T.MeshStandardMaterial({color:0xd5d9dc,roughness:1}));ground.receiveShadow=true;ground.rotation.x=-Math.PI/2;ground.position.y=-.019;scene.add(ground);
  const platform=new T.Mesh(new T.CylinderGeometry(.68,.7,.028,100),new T.MeshStandardMaterial({color:0xeeeeec,metalness:.12,roughness:.67}));platform.receiveShadow=true;platform.position.y=-.016;scene.add(platform);
  ...
  const materialFor=(system:string)=>{
   const m=new T.MeshPhysicalMaterial({color:SYSTEMS.find(s=>s.id===system)?.color??'#aebbb8',metalness:0,roughness:system==='skeletal'?.78:.46,clearcoat:system==='skeletal'?0:.18,clearcoatRoughness:.55,sheen:system==='muscular'?.22:0,sheenColor:new T.Color('#a65960'),side:T.DoubleSide,transparent:system==='integumentary',opacity:system==='integumentary'?.1:1,depthWrite:system!=='integumentary'});
   m.onBeforeCompile=shader=>{
    ...
    shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>','#include <color_fragment>\ndiffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.58, 0.38, 0.15), partSelected * 0.10);');
    shader.fragmentShader=shader.fragmentShader.replace('#include <emissivemap_fragment>','#include <emissivemap_fragment>\nfloat selectionEdge = pow(1.0 - abs(dot(normalize(normal), normalize(vViewPosition))), 3.0); totalEmissiveRadiance += vec3(0.48, 0.28, 0.07) * selectionEdge * partSelected * 0.55;');
  ...
  const clipPlane=new T.Plane(new T.Vector3(0,0,-1),0);let lastCut=-1,lastOpacity=-1;const front=Math.max(...atlas.parts.map(p=>p.bounds[1][2])),back=Math.min(...atlas.parts.map(p=>p.bounds[0][2]));
  let loaded=0;
  const loadChunk=async(ci:number)=>{
   const chunk=atlas.chunks[ci],compressed=!!chunk.gzip&&typeof DecompressionStream!=='undefined';const response=await fetch(`${import.meta.env.BASE_URL}anatomy/models/${(compressed?chunk.gzip!:chunk.url).split('/').pop()}`,{signal:abort.signal});const buffer=await decodeModelResponse(response,chunk.bytes,compressed);if(disposed)return;
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error('Could not assemble anatomy geometry.');geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;mesh.castShadow=system!=='integumentary';mesh.receiveShadow=true;mesh.customDepthMaterial=depthMaterial;scene.add(mesh);});
   lastState=null;loaded++;onProgress(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error?e.message:'Could not load the anatomy.');}})();
  const fit=(view:string,extent=0)=>{
   const h=el.clientHeight, w=el.clientWidth, availableHeight=Math.max(100,h-(latest.current.presentation?145:285));
   const normalDistance=1.9/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2)))*(h/availableHeight);
   const atlasDistance=Math.max(packingHeight*h/availableHeight,packingWidth/camera.aspect)*1.12/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2)));
   ...
   camera.setViewOffset(w,h,0,latest.current.presentation?25:85,w,h);const target=new T.Vector3(0,.85,0);frameCamera(target,target.clone().addScaledVector(direction,distance));dirty=true;
  };
  ...
  const up=(e:PointerEvent)=>{
   ...
   pickers.forEach((mesh,i)=>{if(!mesh||data[i*4+3]<.5||(hasSolid&&atlas.parts[i].system==='integumentary'))return;worldBox.copy(bounds[i]).translate(mesh.position);if(!raycaster.ray.intersectBox(worldBox,hitPoint))return;const hits=raycaster.intersectObject(mesh,false).filter(hit=>!latest.current.cutaway||clipPlane.distanceToPoint(hit.point)>=0);if(hits[0]&&hits[0].distance<nearest){nearest=hits[0].distance;found=i;}});
  ...
  let previous=performance.now(),lastExtent=-1;
  const animate=()=>{
   if(disposed)return;frame=requestAnimationFrame(animate);if(document.hidden){previous=performance.now();return;}const now=performance.now(),dt=Math.min((now-previous)/1000,.05),s=latest.current;previous=now;
   if(lastCut!==(s.cutaway??0)){lastCut=s.cutaway??0;clipPlane.constant=T.MathUtils.lerp(front+.01,back-.01,lastCut);renderer.clippingPlanes=lastCut>0?[clipPlane]:[];dirty=true;}
   if(lastOpacity!==(s.surfaceOpacity??.1)){lastOpacity=s.surfaceOpacity??.1;const surface=mats.get('integumentary');if(surface){surface.opacity=lastOpacity;surface.needsUpdate=true;}dirty=true;}
   if(lastQuality!==s.quality){const high=s.quality==='high';renderer.setPixelRatio(Math.min(devicePixelRatio,high?2.5:1.25));renderer.shadowMap.enabled=high;lastQuality=s.quality??'';dirty=true;}
   if(framing){const t=1-Math.exp(-dt*7);camera.position.lerp(cameraGoal,t);controls.target.lerp(targetGoal,t);if(camera.position.distanceTo(cameraGoal)<.0002&&controls.target.distanceTo(targetGoal)<.0002){camera.position.copy(cameraGoal);controls.target.copy(targetGoal);framing=false;}dirty=true;}
   const changed=lastState?.visible!==s.visible||lastState?.selected!==s.selected||lastState?.isolate!==s.isolate;
   const moving=Math.abs(amount-s.explode)>.0001;
   if(moving){amount=s.reducedMotion?s.explode:T.MathUtils.damp(amount,s.explode,8,dt);dirty=true;}
   ...
   if(isolateKey!==lastIsolate||(s.isolate&&moving)){
    if(s.isolate){const box=new T.Box3();atlas.parts.forEach((p,i)=>{if(s.selected.includes(p.id))box.union(bounds[i].clone().translate(new T.Vector3(data[i*4],data[i*4+1],data[i*4+2])));});
     if(!box.isEmpty()){const center=box.getCenter(new T.Vector3()),size=box.getSize(new T.Vector3());const w=el.clientWidth,h=el.clientHeight;const left=20,right=w-20,top=55,bottom=h-225;const availableWidth=Math.max(150,right-left),availableHeight=Math.max(40,bottom-top);camera.setViewOffset(w,h,w/2-(left+right)/2,h/2-(top+bottom)/2,w,h);const distance=Math.max(.07,Math.max(size.y*h/availableHeight,size.x*w/availableWidth/camera.aspect,size.z)/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2)))*1.35);controls.maxDistance=Math.max(40,distance*2);frameCamera(center,center.clone().add((s.view==='front'?new T.Vector3(0,0,1):s.view==='back'?new T.Vector3(0,0,-1):s.view==='side'?new T.Vector3(1,0,0):new T.Vector3(.2,.1,1).normalize()).multiplyScalar(distance)));dirty=true;}
    }else if(lastIsolate){camera.clearViewOffset();fit(s.view,amount);}
    lastIsolate=isolateKey;
   }
   controls.enableRotate=amount<.8;controls.mouseButtons.LEFT=amount<.8?T.MOUSE.ROTATE:T.MOUSE.PAN;controls.touches.ONE=amount<.8?T.TOUCH.ROTATE:T.TOUCH.PAN;ground.visible=platform.visible=ring.visible=innerRing.visible=amount<.5&&!s.isolate;markers.visible=amount>.75;controls.autoRotate=!framing&&s.rotate&&!s.reducedMotion&&!s.isolate&&amount<.4;controls.autoRotateSpeed=.65;controls.update();if(controls.autoRotate)dirty=true;
