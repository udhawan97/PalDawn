...
assert.equal(packageJson.version, '0.4.0', 'package version must match the release')
...
assert.doesNotMatch(source, /\.(glb|gltf|fbx|obj|blend)(?:[?"'])/i, 'unreviewed anatomy/binary assets are not allowed in v0.4')
assert.doesNotMatch(source, /[\u0900-\u097f]/u, 'the v0.4 application surface must remain English-only')
