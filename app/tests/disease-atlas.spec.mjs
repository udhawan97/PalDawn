...
test('Atlas Study Desk filters local work, discovers authored systems, and exposes exact evidence links', async ({ page }) => {
  await page.goto('./#atlas/diabetes/pancreas-senses')
  await page.getByRole('button', { name: 'Save step' }).click()
  await page.getByRole('button', { name: 'Private note' }).click()
  ...
  await desk.getByLabel('Find a saved step or private note').fill('insulin control')
  await expect(desk.getByRole('heading', { name: 'The pancreas releases insulin' })).toBeVisible()

  await page.emulateMedia({ media: 'print' })
  await expect(desk.getByText('Review the insulin control signal.')).not.toBeVisible()
  await page.emulateMedia({ media: 'screen' })
  await desk.getByRole('checkbox', { name: 'Include private notes in download and print' }).check()
  await page.emulateMedia({ media: 'print' })
  await expect(desk.getByText('Review the insulin control signal.')).toBeVisible()
  await page.emulateMedia({ media: 'screen' })
  const downloadEvent = page.waitForEvent('download')
  await desk.getByRole('button', { name: 'Download complete study' }).click()
  const markdown = await readFile(await (await downloadEvent).path(), 'utf8')
  expect(markdown).toContain('Review the insulin control signal.')
  expect(markdown).toContain('The pancreas releases insulin')

  await desk.getByRole('button', { name: 'Body systems' }).click()
  await desk.getByRole('button', { name: /^Pancreas / }).click()
  const pancreasResult = desk.locator('.atlas-system-results li').filter({ hasText: 'The pancreas releases insulin' })
  await pancreasResult.getByRole('button', { name: 'Open exact step' }).click()
  await expect(page.getByRole('heading', { name: 'The pancreas releases insulin' })).toBeVisible()
  ...
  await expect(desk.getByRole('link', { name: 'Open source' })).toHaveAttribute('href', /niddk\.nih\.gov/)
  await expect(desk.getByRole('button', { name: 'Food enters the digestive tract' })).toBeVisible()
})
...
test('Atlas Study Desk stays bounded on a narrow screen and restores focus', async ({ page }) => {
  ...
  await expect(desk.getByRole('button', { name: 'Close Atlas study desk' })).toBeFocused()
