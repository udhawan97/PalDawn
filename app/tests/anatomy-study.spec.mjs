...
test('Anatomy study IDs, read marks and unavailable records survive reload and export', async ({ page }) => {
  ...
  await expect(page.getByText('0 read')).toBeVisible()
  await expect(page.getByText('2 unread')).toBeVisible()
  await page.getByRole('button', { name: 'Unread 2' }).click()
  await expect(page.getByRole('link', { name: /Continue next unread/ })).toHaveAttribute('href', /^https:\/\//)
  ...
  expect(backup.study.lastSelection.male).toBe('FMA7088')
})
...
test('Anatomy clear remains available for a resume-only record', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Text study' }).click()
  await page.getByText('Saved Anatomy study & backup').click()
  await page.getByRole('button', { name: 'Clear local Anatomy study' }).click()
  await page.getByRole('button', { name: 'Confirm clear Anatomy study' }).click()
